...
anno_new_analysis <- read.delim("E:/RNA-Seq/Covid/STAR_more_samples/anno_rev.txt")
sample_table_sub <- merge(anno_new_analysis, sample_table[,c(1,2,32)], by = "ID")

sample_table_sub$who_bins <- factor(sample_table_sub$who_bins,levels = rev(c("control","mild","severe")))
sample_table_sub$new_cluster <- factor(sample_table_sub$new_cluster,levels = c(1,2,3,4,5,6))

sample_table_sub$WHO <- as.character(sample_table_sub$WHO)
sample_table_sub$WHO <- factor(sample_table_sub$WHO,levels = rev(c("0","3","4","6","7")))
sample_table_sub$new_cluster <- factor(sample_table_sub$new_cluster,levels = rev(c(1,2,3,4,5,6)))



  ggplot(sample_table_sub,
         aes(axis1 = status, axis2 = who_bins ,axis3 = new_cluster)) +
    geom_alluvium(aes(fill = new_cluster), width = 1/12, color = "black") +
